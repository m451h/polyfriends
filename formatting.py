"""
All text formatting and keyboard builders.

Key rule:
  - market_card()       → used in real bot messages (supports Markdown)
  - market_card_plain() → used in InputTextMessageContent (NO Markdown, ever)
"""
...
# ── Cards for real bot messages (Markdown OK) ─────────────────────────────────

def market_card(m: dict) -> str:
    icon = {"pending":"⏳","open":"🟢","closed":"🔴",
            "resolved":"✅","rejected":"❌","cancelled":"↩️"}.get(m["status"],"❓")
    yes, no, tot, yp = _pools(m)
    np = 100 - yp
    lines = [
        f"{icon} *{_safe(m['question'])}*",
        f"`{_bar(yp)}`",
...
def leaderboard_msg(rows: list, group_name: str) -> str:
    medals = ["🥇","🥈","🥉"]
    lines  = [f"🏆 *{_safe(group_name)} Leaderboard*\n"]
    for i, r in enumerate(rows):
        medal = medals[i] if i < 3 else f"{i+1}\\."
        lines.append(f"{medal} {_safe(r['username'])}  {r['balance']:.1f} pts")
    return "\n".join(lines)

# ── Plain-text cards for InputTextMessageContent (zero Markdown) ──────────────

def market_card_plain(m: dict) -> str:
    """Safe for InputTextMessageContent — absolutely no Markdown."""
    icon = {"pending":"⏳","open":"🟢","closed":"🔴",
            "resolved":"✅","rejected":"❌","cancelled":"↩️"}.get(m["status"],"❓")
    yes, no, tot, yp = _pools(m)
    np = 100 - yp
...
def balance_plain(username: str, groups: list) -> str:
    if not groups:
        return f"{username} - not in any groups yet. Use /join in a group."
...
# ── Keyboards ─────────────────────────────────────────────────────────────────
# Every callback_data ends with _<group_id> so callbacks work on inline messages.

def kb_load(market_id: int, group_id: int) -> InlineKeyboardMarkup:
    """Single button on the inline-posted placeholder. Tapping it loads the real card."""
    return InlineKeyboardMarkup([[
        InlineKeyboardButton("Tap to load market", callback_data=f"load_{market_id}_{group_id}")
    ]])
...
def kb_sell_amounts(side: str, market_id: int, group_id: int, max_shares: float) -> InlineKeyboardMarkup:
    g, m = group_id, market_id
    q25, q50, q100 = round(max_shares*.25,3), round(max_shares*.5,3), round(max_shares,3)
    return InlineKeyboardMarkup([
        [
